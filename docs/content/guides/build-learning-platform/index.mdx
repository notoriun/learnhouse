...
- **A running LearnHouse instance.** Spin one up locally with the [Quickstart](/getting-started/quickstart) or the [self-hosting guide](/self-hosting). The examples assume the API is at `http://localhost:1338`.
...
Throughout this guide the API base URL is `http://localhost:1338/api/v1`. If your instance lives elsewhere (a custom port or domain), substitute your own base URL — every example reads it from a single environment variable.
</Callout>
