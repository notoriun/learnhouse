...
- [X] T001 Atualizar `docker/keycloak/realm-dev.json`: `registrationAllowed: true` e
...
- [X] T002 [US1] Em `apps/api/src/services/auth/keycloak_oidc.py`, aceitar
...
- [X] T003 [US1] Em `apps/api/src/routers/keycloak_auth.py`, adicionar campo opcional
...
- [X] T004 [US1] Testes em `apps/api/src/tests/routers/test_keycloak_auth_router.py`:
...
- [X] T005 [P] [US1] Em `apps/web/app/api/auth/keycloak/authorize/route.ts`, repassar
...
- [X] T006 [P] [US1] Em `apps/web/app/auth/login/login.tsx`, exibir "Criar conta pela
...
- [X] T007 [US1] Validar quickstart passo 1 no ambiente docker local (registro feliz +
...
- [X] T008 [US2] Criar `apps/api/scripts/migrate_users_to_keycloak.py` conforme
      ...
      logados
- [X] T009 [US2] Testes em `apps/api/src/tests/scripts/test_migrate_users.py` (httpx
...
- [X] T010 [US2] Validação prática do import Argon2 no docker local (quickstart passo 2):
...
- [X] T011 [P] [US3] Criar `apps/api/src/services/auth/federation.py`:
      ...
      (`<issuer>/account`)
- [X] T012 [US3] Em `apps/api/src/services/users/users.py`, aplicar o guard nas duas
...
- [X] T013 [US3] Testes em `apps/api/src/tests/services/test_federated_guards.py`: conta
...
- [X] T014 [P] [US3] Em `apps/web/components/Dashboard/Pages/Users/Security/`, para conta
...
- [X] T015 [US3] Validar quickstart passo 3 (API, UI, sync de e-mail no login, conta de
      controle)
...
- [X] T016 Rodar a suíte completa da API (`uv run pytest`) e o typecheck do web
      (`bun run typecheck` em `apps/web`) — tudo verde (quickstart passo 4)
- [X] T017 Executar o quickstart completo ponta a ponta no ambiente docker local e conferir
